...
interface ProductTableProps {
	...
	onToggleStatus?: (id: string, currentStatus: string) => void; // Thêm prop này
	isDeleting?: boolean;
	updatingStatusId?: string | null; // Thêm prop id đang loading
}
				...
				<tbody className="divide-y divide-brand-border/60">
					{products.map((product) => (
						<ProductRow
							...
							onEdit={onEdit}
							onDelete={onDelete}
							onToggleStatus={onToggleStatus} // Truyền xuống ProductRow
							isDeleting={isDeleting}
							isUpdatingStatus={updatingStatusId === product.id} // Kiểm tra đúng sản phẩm đang xử lý
						/>
					))}
				</tbody>
			</table>
