...
export function ProductReviewsSection({ productId }: ProductReviewsSectionProps) {
  ...
  const { data: reviewsData, isLoading: isReviewsLoading } = useProductReviewsQuery(productId, page, 5, activeTab);
  const { data: summary, isLoading: isSummaryLoading } = useProductReviewsSummaryQuery(productId);
  const addReviewMutation = useAddProductReviewMutation(productId);

  const [lightboxMedia, setLightboxMedia] = useState<string | null>(null);

  // Xử lý Upload file qua Presigned URL
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        // 2. Upload trực tiếp lên S3/MinIO qua link presigned
        ...
        // 3. Đưa URL public vào list hiển thị
  ...
  // Tính phần trăm cho biểu đồ cột
  ...
  // Sử dụng trực tiếp danh sách trả về từ API đã lọc theo rating từ backend
  ...
  return (
    <div className="space-y-8 py-8 border-t border-brand-border text-xs font-sans text-brand-dark animate-in fade-in duration-300">
      ...
      {/* 4. Lightbox/Modal Viewer for high quality video/photo zoom */}
      {lightboxMedia && (
