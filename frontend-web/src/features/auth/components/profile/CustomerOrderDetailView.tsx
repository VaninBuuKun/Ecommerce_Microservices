...
import { Loader2, AlertTriangle, Upload, X, Image as ImageIcon, Video as VideoIcon } from "lucide-react";
import { 
...
export function CustomerOrderDetailView({ subOrderId, onBack }: CustomerOrderDetailViewProps) {
	...
	const orderTimeStr = detail.createdDate 
		? new Date(detail.createdDate).toLocaleTimeString("vi-VN", { hour: '2-digit', minute: '2-digit' }) + " " + new Date(detail.createdDate).toLocaleDateString("vi-VN")
		: "N/A";
	...
	const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		try {
			...
			setUploadedMedias((prev) => [...prev, ...urls]);
		} catch (err: any) {
			console.error("Lỗi khi tải file lên S3", err);
			setErrorMessage("Không thể tải tập tin minh chứng lên S3. Vui lòng thử lại!");
		} finally {
	...
	const handleRefundOrder = () => {
		...
		refundMutation.mutate(
			{ subOrderId: detail.id, reason: reason.trim(), medias: uploadedMedias.length > 0 ? uploadedMedias : undefined },
			{
				onSuccess: () => {
				...
				onError: (err: any) => {
					const errorMsg = err?.response?.data?.message || err?.response?.data || "Đã xảy ra lỗi khi yêu cầu hoàn tiền.";
			...
			{showRefundModal && (
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-dark/40 backdrop-blur-xs font-sans">
					<div className="bg-white rounded-xl max-w-md w-full border border-brand-border p-5 shadow-xl space-y-4">
						...
						<div className="space-y-1">
							<label className="block text-[11px] font-bold text-brand-muted uppercase">Lý do hoàn trả <span className="text-red-500">*</span></label>
							<textarea
								value={reason}
								onChange={(e) => setReason(e.target.value)}
								placeholder="Nhập lý do trả hàng/hoàn tiền chi tiết..."
								rows={3}
								className="w-full border border-brand-border rounded-lg p-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-brand-primary font-medium"
							/>
						</div>
						<div className="space-y-2">
							<label className="block text-[11px] font-bold text-brand-muted uppercase">Hình ảnh / Video minh chứng (Upload S3 Storage)</label>
							
							{/* Danh sách các media đã upload thành công */}
							{uploadedMedias.length > 0 && (
								<div className="grid grid-cols-4 gap-2 border border-brand-border p-2 rounded-lg bg-brand-light-soft/20">
									{uploadedMedias.map((url, idx) => {
										const isVideo = url.match(/\.(mp4|webm|ogg|mov)$/i);
										return (
											<div key={idx} className="relative group aspect-square rounded-md overflow-hidden border border-brand-border bg-slate-100 flex items-center justify-center">
												{isVideo ? (
												...
												<button
													...
													className="absolute top-1 right-1 bg-red-600 text-white p-0.5 rounded-full opacity-90 hover:opacity-100 cursor-pointer shadow"
													title="Xóa tệp này"
												>
													<X className="w-3 h-3" />
												</button>
							...
							{/* Nút Upload trực tiếp lên S3 */}
							<label className="flex items-center justify-center gap-2 p-3 border-2 border-dashed border-brand-border hover:border-brand-primary rounded-lg bg-brand-light-soft/10 hover:bg-brand-primary/5 transition-all cursor-pointer">
								<input
									type="file"
									multiple
									accept="image/*,video/*"
								...
								{isUploading ? (
									<div className="flex items-center gap-2 text-xs text-brand-muted font-bold">
										<Loader2 className="w-4 h-4 animate-spin text-brand-primary" /> Đang tải tệp lên S3 Storage...
									</div>
								) : (
									<div className="flex items-center gap-2 text-xs text-brand-muted font-extrabold">
										<Upload className="w-4 h-4 text-brand-primary" />
										<span>Bấm để tải ảnh/video từ máy tính lên S3</span>
									</div>
								)}
							</label>
							<p className="text-[10px] text-brand-muted font-bold">* Hỗ trợ ảnh JPG, PNG, WEBP và Video MP4, MOV (Upload trực tiếp S3 Storage).</p>
						</div>
						{errorMessage && <p className="text-[10px] font-bold text-red-600">{errorMessage}</p>}
						<div className="flex justify-end gap-2 pt-2">
							<button
