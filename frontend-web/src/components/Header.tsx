...
import { ShoppingBag, Search, LogOut, Package, Settings, Store, Bell } from 'lucide-react';
...
export default function Header() {
  ...
  const mockCartItems = [
    { id: 1, name: 'Áo Hoodie Streetwear Emerald Accent', price: '450.000đ', image: 'https://images.unsplash.com/photo-1556821840-3a63f95609a7?auto=format&fit=crop&q=80&w=150' },
    { id: 2, name: 'Quần Cargo Đen Technical Kaki', price: '520.000đ', image: 'https://images.unsplash.com/photo-1542272604-787c3835535d?auto=format&fit=crop&q=80&w=150' },
  ];
  ...
  return (
    <header className="sticky top-0 z-50 w-full h-14 bg-white/80 backdrop-blur-md border-b border-brand-border px-6 flex items-center justify-between shadow-[0_1px_3px_rgba(0,0,0,0.02)] font-sans">
      ...
      <div className="flex items-center gap-40 flex-1 min-w-0 pr-4">
        {/* Logo */}
        <Link to="/" className="flex items-center gap-2 font-bold text-lg text-brand-dark tracking-tight shrink-0 h-8">
          <img
          ...
          />
          <span className="font-extrabold text-brand-dark tracking-tighter leading-none">BUU<span className="text-brand-primary font-medium">STORE</span></span>
        </Link>

        {/* Thanh Search thông minh (Chiều cao h-8 chuẩn) */}
        <div ref={searchRef} className="relative w-full max-w-2xl hidden sm:block">
          ...
          {showSearchSuggestions && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-brand-border rounded shadow-lg p-2 text-left z-50">
              <span className="block text-[10px] text-brand-muted font-bold px-2 py-1 uppercase tracking-wider">Tìm kiếm phổ biến</span>
              <div className="mt-1">
                {mockSuggestions
              ...
              {showCartDropdown && (
                <div className="absolute right-0 top-full pt-1.5 w-72 z-50">
                  <div className="bg-white border border-brand-border rounded shadow-xl p-3 text-left">
                    <span className="block text-xs font-bold text-brand-dark border-b border-brand-border pb-2 mb-2">Giỏ hàng của tôi</span>

                    <div className="space-y-3 max-h-48 overflow-y-auto">
                      {mockCartItems.map((item) => (
                        <div key={item.id} className="flex items-center gap-3">
                          <img src={item.image} alt={item.name} className="w-10 h-10 object-cover rounded border border-brand-border" />
                          <div className="flex-1 min-w-0">
                            <h4 className="text-xs font-bold text-brand-dark truncate">{item.name}</h4>
                            <span className="text-[10px] text-brand-primary font-bold">{item.price}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="bg-white border border-brand-border rounded shadow-xl p-3 z-50">
                  <div className="flex items-center gap-3 pb-3 border-b border-brand-border mb-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <span className="text-xs text-brand-muted truncate block text-left">{user.email}</span>
                    </div>
                  </div>
