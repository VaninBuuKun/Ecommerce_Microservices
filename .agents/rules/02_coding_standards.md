...
- **AppSettings Cascading Configuration Rule**:
  - `appsettings.json` is committed with skeleton and placeholders (`YOUR_*`) as the base and example. Never commit real secrets.
  - `appsettings.Developer.json` is gitignored (`*.Developer.json`) and loaded automatically via `AddCustomConfiguration()` in `BuildingBlocks.Logging`. Deep merges local secrets over `appsettings.json`.
