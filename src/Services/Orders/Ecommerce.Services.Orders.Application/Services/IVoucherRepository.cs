...
public interface IVoucherRepository
{
    ...
    Task<bool> TryIncrementUsagesAsync(ICollection<long> voucherIds, CancellationToken cancellationToken = default);

    /// <summary>
    /// Giảm UsageCount xuống 1 một cách atomic khi hoàn lại voucher do hủy đơn.
    /// Đảm bảo không giảm xuống dưới 0.
    /// </summary>
    Task DecrementUsageAsync(long voucherId, CancellationToken cancellationToken = default);

    /// <summary>
    /// Giảm UsageCount xuống 1 một cách atomic cho danh sách voucher trong 1 query duy nhất.
    /// Đảm bảo không giảm xuống dưới 0.
    /// </summary>
    Task DecrementUsagesAsync(ICollection<long> voucherIds, CancellationToken cancellationToken = default);
}
